...
public class Join extends JobMapReduce {
    ...
    public static class JoinMapper extends Mapper<Text, Text, IntWritable, Text> {
        ...
        /*
         Constante que utilizaremos para calcular el valor de la key.
         */
        ...
        public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
			...
			/*
			Obtenemos el valor del parametro join y si se cumplen las condiciones generamos una key para separarlos.
			 */
            String joinValue = Utils.getAttribute(arrayValues, join);
            if (joinValue.equals(external)) {
				/*
				Si el valor es external calculamos una key aleatória que sea múltiplo de N y la escribimos junto con
				su valor asociado.
				 */
				...
				/*
				Si el valor es internal escribimos el valor N veces, con keys des de 0 hasta N-1.
				 */
    ...
    public static class JoinReducer extends Reducer<IntWritable, Text, NullWritable, Text> {
        ...
        public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			/*
			Finalmente recorremos los sets comprobando que las regiones coincidan, y en caso afirmativo escribos el
			join de las filas.
			 */
            for (int i = 0; i < externals.size(); i++) {
                String[] externalValues = externals.get(i).split(",");
                String externalRegion = Utils.getAttribute(externalValues, region);
                for (int j = 0; j < internals.size(); j++) {
                    String[] internalValues = internals.get(j).split(",");
                    if (externalRegion.equals(Utils.getAttribute(internalValues, region))) {
    ...
    public static void configureJob(Job job, String pathIn, String pathOut)
		...
		/*
        Finalmente le pasamos los parámetros necesarios a la configuración, en esta ocasión primero le pasamos los del
        join, en este caso el atributo type, el valor del external, en este caso type_1, el valor del internal, en
        este caso type_2, y el atributo que utilezaremos para el reducer, en este caso region.
         */
